...
const AGAIN_STEPS_MS = [60000, 90000, 120000, 180000, 240000, 300000];
const HARD_STEPS_MS = [600000, 720000, 900000, 1200000];
...
class StudyEngine {
  ...
  rate(cardId, quality, timeSpentSeconds, now = new Date()) {
    ...
    if (quality === 0) {
      card.lapses += 1;
      card.learningState = 'learning';
      card.stability = S_MIN;
      card.retrievability = 0;
      card.difficulty = clamp(card.difficulty + 0.2, 1, 10);
      card.shortTermReps = (card.shortTermReps || 0) + 1;
      card.lastReview = now.toISOString();
      const delay = this.getShortTermDelay(card, 0);
      const due = new Date(now.getTime() + delay);
      card.nextReview = due.toISOString();
      this.enqueueShortTerm(card, due);
    } else if (quality === 1) {
      card.learningState = 'learning';
      card.stability = Math.max(card.stability * 0.75, S_MIN);
      card.retrievability = Math.max(card.retrievability * 0.6, 0);
      card.difficulty = clamp(card.difficulty + 0.3, 1, 10);
      card.shortTermReps = (card.shortTermReps || 0) + 1;
      card.lastReview = now.toISOString();
      const delay = this.getShortTermDelay(card, 1);
      const due = new Date(now.getTime() + delay);
      card.nextReview = due.toISOString();
      this.enqueueShortTerm(card, due);
    } else {
