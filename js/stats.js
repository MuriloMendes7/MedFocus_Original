...
const Stats = {
    ...
    init: () => {
        ...
        // Inicializar integração com cronômetro
        if (typeof StudyTimer !== 'undefined') {
            Stats.timer.updateTimeSummaryCards(Stats.currentUser);
            Stats.charts.studyTime = Stats.timer.createStudyTimeChart();
        }
    },
    ...
    timer: {
        ...
        getStudyTimeStats: (userId, period = 30) => {
            if (typeof StudyTimer !== 'undefined') {
                return StudyTimer.getTimeStats(userId, null, period);
            }
            return { totalTime: 0, totalSessions: 0, averageTime: 0 };
        },

        // Obter tempo de estudo hoje
        getTodayStudyTime: (userId) => {
            if (typeof StudyTimer !== 'undefined') {
                return StudyTimer.getTodayStudyTime(userId);
            }
            return { totalTime: 0, formattedTime: '00:00' };
        },

        // Obter tempo de estudo por deck
        getDeckStudyTime: (userId, deckId, period = 30) => {
            if (typeof StudyTimer !== 'undefined') {
                return StudyTimer.getTimeStats(userId, deckId, period);
            }
            return { totalTime: 0, totalSessions: 0, averageTime: 0 };
        },

        // Atualizar cards de resumo com dados de tempo
        updateTimeSummaryCards: (userId) => {
            const todayStats = Stats.timer.getTodayStudyTime(userId);
            const periodStats = Stats.timer.getStudyTimeStats(userId, 30);

            // Atualizar card de tempo hoje
            const todayTimeCard = document.getElementById('todayStudyTime');
            if (todayTimeCard) {
                const timeElement = todayTimeCard.querySelector('.stat-value');
                if (timeElement) {
                    timeElement.textContent = todayStats.formattedTime;
                }
            }

            // Atualizar card de tempo total
            const totalTimeCard = document.getElementById('totalStudyTime');
            if (totalTimeCard) {
                const timeElement = totalTimeCard.querySelector('.stat-value');
                if (timeElement) {
                    timeElement.textContent = periodStats.formattedTotalTime;
                }
            }

            // Atualizar card de sessões
            const sessionsCard = document.getElementById('totalSessions');
            if (sessionsCard) {
                const sessionsElement = sessionsCard.querySelector('.stat-value');
                if (sessionsElement) {
                    sessionsElement.textContent = periodStats.totalSessions;
                }
            }
        },

        // Criar gráfico de tempo de estudo
