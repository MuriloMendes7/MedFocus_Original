    ...
    execution: {
        ...
        saveResults: (results) => {
            ...
            if (quizIndex !== -1) {
                ...
                if (results.score > quiz.bestScore) {
                    quiz.bestScore = results.score;
                }
                
                // Calcular score médio
                const previousTotal = quiz.averageScore * (quiz.attempts - 1);
                quiz.averageScore = Math.round((previousTotal + results.score) / quiz.attempts);
                
                // Atualizar tempo médio
                const previousTimeTotal = quiz.stats.averageTime * (quiz.attempts - 1);
                quiz.stats.averageTime = Math.round((previousTimeTotal + results.timeSpent) / quiz.attempts);
                
                Storage.setUserQuizzes(userId, quizzes);
            }

            // Salvar resultado individual
            const userResults = Storage.getUserData(userId, 'quiz_results') || [];
            userResults.push(results);
            Storage.setUserData(userId, 'quiz_results', userResults);
            
            // Atualizar estatísticas gerais do usuário
            Quizzes.execution.updateUserStats(results);
        },
        ...
        updateUserStats: (results) => {
            ...
            const dayStats = userStats[today];
            dayStats.quizzes = (dayStats.quizzes || 0) + 1;
            dayStats.quizQuestions = (dayStats.quizQuestions || 0) + results.totalQuestions;
            dayStats.quizCorrect = (dayStats.quizCorrect || 0) + results.correctAnswers;
            dayStats.quizTime = (dayStats.quizTime || 0) + results.timeSpent;

            Storage.setUserStats(userId, userStats);
        },
